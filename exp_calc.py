...
class MyExcelfile:
    ...
    def get_transaction(cls,sdate,edate):
        ...
        if filtered_df.empty:
           return {"message" : "There are no transactions done in the given date range."}

        transactions = []
        for index,row in filtered_df.iterrows():
            transaction={"date" : row["date"].strftime('%m-%d-%Y'),
        ...
        income_dict, expense_dict = plot_transaction(filtered_df)

        return {"message": f"Transactions from {sdate.strftime(MyExcelfile.dateFORMAT)} to {edate.strftime(MyExcelfile.dateFORMAT)}",
                "transactions" : transactions,
                "Summary":{"Total Income": total_income,
                            "Total Expense": total_expense,
                            "Net Savings" : net_savings},
                "income_plot_data":income_dict,
                "expense_plot_data":expense_dict
                }
...
def adddata():
    ...
    date = (get_date("Enter the date of Transaction in 'mm-dd-yyyy' format or press enter for today's date: ", allow_default= True)).strftime('%m-%d-%Y')
...
def plot_transaction(df):
    ...
    # income_df['new_date'] = income_df['date'].dt.strftime('%Y-%m-%d')
    # income_df.set_index('new_date', inplace = True)
    # expense_df['new_date'] = expense_df['date'].dt.strftime('%Y-%m-%d')
    # expense_df.set_index('new_date', inplace = True)
    ...
    for index, row in income_df.iterrows():
        income_dict[row.name.strftime("%m-%d-%Y")] = row["amount"]

    expense_dict = dict()
    for index, row in expense_df.iterrows():
        expense_dict[row.name.strftime("%m-%d-%Y")] = row["amount"]
